...
void Sprite::Draw( SpriteBatch* pBatch )
{
	if ( pTexture )
	{
		...
		if (bMirror)
			effect = DirectX::SpriteEffects_FlipHorizontally;

		pBatch->Draw( pTexture->GetResourceView(), position, &textureRegion ,color, rotation, origin, scale, effect, layer );
	}
}
...
void Sprite::SetTextureAnimation(int frameSizeX, int frameSizeY, int framesPerSecond, int numFrames)
{
	if (pTexture == nullptr)
		return;
