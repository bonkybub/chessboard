...
void King::Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, LitColourShader* pLitShader, bool white, TextureType* mainTex, TextureType* accTex, float bOffset)
{
	pShader = pLitShader;
	...
	headCone.InitializeGeometry(pDevice, Cone);
	headCone.InitializeInputLayout(pDevice, pShader->GetVertexShaderBinary(), pShader->GetVertexShaderBinarySize());
	...
	tipTorus.InitializeGeometry(pDevice, Torus);
	tipTorus.InitializeInputLayout(pDevice, pShader->GetVertexShaderBinary(), pShader->GetVertexShaderBinarySize());

	tipCube1.InitializeGeometry(pDevice, Cube);
	tipCube1.InitializeInputLayout(pDevice, pShader->GetVertexShaderBinary(), pShader->GetVertexShaderBinarySize());

	tipCube2.InitializeGeometry(pDevice, Cube);
	tipCube2.InitializeInputLayout(pDevice, pShader->GetVertexShaderBinary(), pShader->GetVertexShaderBinarySize());
	...
	headConeMatrix = Matrix::CreateScale(1.02f, 1.9f, 1.02f) *
		Matrix::CreateRotationX(XM_PI) * 
		Matrix::CreateTranslation(0.0f, 1.2f, 0.0f);

	headSphereMatrix = Matrix::CreateScale(0.91f, 0.33f, 0.91f) *
		Matrix::CreateTranslation(0.0f, 2.1f, 0.0f);

	tipTorusMatrix = Matrix::CreateScale(0.18f, 0.23f, 0.18f) *
		Matrix::CreateTranslation(0.0f, 2.28f, 0.0f);

	tipCubeMatrix1 = Matrix::CreateScale(0.11f, 0.31f, 0.09f) *
		Matrix::CreateTranslation(0.0f, 2.49f, 0.0f);

	tipCubeMatrix2 = Matrix::CreateScale(0.11f, 0.32f, 0.09f) *
...
void King::Draw(ID3D11DeviceContext* pDeviceContext, const Matrix& parentMatrix, const Matrix& viewMatrix, const Matrix& projMatrix)
{
	...
	pDeviceContext->PSSetConstantBuffers(2, 1, &pMainBuffer);
	pDeviceContext->PSSetShaderResources(0, 1, &pTex1);
	pShader->SetShaders(pDeviceContext, headConeMatrix * parentMatrix, viewMatrix, projMatrix);
	headCone.Draw(pDeviceContext);
	...
	pDeviceContext->PSSetConstantBuffers(2, 1, &pAccentBuffer);
	pDeviceContext->PSSetShaderResources(0, 1, &pTex2);
	pShader->SetShaders(pDeviceContext, tipTorusMatrix * parentMatrix, viewMatrix, projMatrix);
	tipTorus.Draw(pDeviceContext);

	pDeviceContext->PSSetConstantBuffers(2, 1, &pMainBuffer);
	pDeviceContext->PSSetShaderResources(0, 1, &pTex1);
	pShader->SetShaders(pDeviceContext, tipCubeMatrix1 * parentMatrix, viewMatrix, projMatrix);
	tipCube1.Draw(pDeviceContext);
	
	pShader->SetShaders(pDeviceContext, tipCubeMatrix2 * parentMatrix, viewMatrix, projMatrix);
	tipCube2.Draw(pDeviceContext);
}
